...
const HRUploadModal: React.FC<HRUploadModalProps> = ({ isOpen, onClose, onUploadSuccess, onError }) => {
  ...
  const handleDownloadTemplate = async () => {
    const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/template/hr/?format=excel`;
    try {
      ...
      const fileName = `CV_hr_list_${timestamp}.xlsx`;
      ...
      if (onError) onError("Failed to download HR template. Please try again.");
      onClose();
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/upload-hr/`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        ...
        try {
          ...
          } else if (errorMessage.includes("Invalid hr_id format") || 
                     errorMessage.includes("hr_id must be alphanumeric")) {
            errorMessage = "HR ID can contain letters, numbers, and hyphens (-)";
        ...
        <div className="flex-1 flex flex-col items-center justify-between p-4 sm:p-8 bg-gradient-to-b from-blue-50/60 to-white dark:from-blue-900/30 dark:to-[#181E2A] min-h-[180px]">
          <div className="flex flex-col items-center w-full">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 text-center">
              Use this template to upload HR data to the CRM.
            </p>
          </div>
          <Button
