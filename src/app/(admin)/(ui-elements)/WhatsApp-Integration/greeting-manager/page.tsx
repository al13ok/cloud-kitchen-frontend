...
const ADMIN_PREFIX = `${BASE_API_URL}/admin/greetings`;
...
export default function GreetingManagerPage(): JSX.Element {
  ...
  const fetchMenuTriggers = useCallback(async () => {
    try {
      const res = await fetch(`${BASE_API_URL}/admin/menu-triggers`);
  ...
  async function addMenuTrigger() {
    ...
    if (!value) return;
    try {
      const res = await fetch(`${BASE_API_URL}/admin/menu-triggers`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ word: value }) });
  ...
  async function removeMenuTrigger(value: string) {
    try {
      ...
      const res = await fetch(`${BASE_API_URL}/admin/menu-triggers?word=${encodeURIComponent(value)}`, { method: "DELETE", headers: { accept: "application/json" } });
      if (!res.ok) {
        // revert on failure
        await fetchMenuTriggers();
  ...
  async function replaceMenuTriggers(words: string[]) {
    try {
      ...
      const res = await fetch(`${BASE_API_URL}/admin/menu-triggers`, { method: "PUT", headers: { "Content-Type": "application/json", accept: "application/json" }, body: JSON.stringify({ words: unique }) });
