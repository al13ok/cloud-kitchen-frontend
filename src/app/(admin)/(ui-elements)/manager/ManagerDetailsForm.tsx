...
const managerSchema = z.object({
  manager_id: z
    ...
    .max(20, { message: "Manager ID must be at most 20 characters" })
    .regex(/^[a-zA-Z0-9\-]+$/, { message: "Manager ID can contain letters, numbers, and hyphens (-)" }),
  name: z
    .string()
...
const ManagerDetailsForm: React.FC<ManagerDetailsFormProps> = ({ onSuccess }) => {
  ...
  const checkEmailDuplicate = useCallback(async (email: string) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/v1/manager/?page=1&size=1000`);
      if (response.ok) {
        const data = await response.json();
        const managerList = Array.isArray(data.data) ? data.data : [];
        const existingManager = managerList.find((manager: { email?: string }) =>
          manager.email && manager.email.toLowerCase() === email.toLowerCase()
        );
  ...
  // Function to check if Manager ID already exists
  const checkManagerIdDuplicate = useCallback(async (managerId: string) => {
    if (!managerId) return;
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const managerList = Array.isArray(data.data) ? data.data : [];
        const existingManager = managerList.find((manager: { manager_id?: string | number }) =>
          manager.manager_id && String(manager.manager_id).toLowerCase() === managerId.toLowerCase()
        );

        if (existingManager) {
          setError("manager_id", {
            type: "manual",
            message: "This Manager ID is already taken. Please use a different ID."
          });
        } else {
      ...
      console.error("Error checking Manager ID duplicate:", error);
    } finally {
  ...
  // Debounced validation for Manager ID
  React.useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (watchedManagerId) {
        checkManagerIdDuplicate(watchedManagerId);
      }
    }, 500);
    return () => clearTimeout(timeoutId);
  }, [watchedManagerId, checkManagerIdDuplicate]);
  ...
  const onSubmit = async (data: ManagerFormValues) => {
    try {
      const response = await fetch(`${BASE_URL}/api/v1/create-manager/`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        if (err.detail && err.detail.includes("E11000 duplicate key error")) {
          ...
          } else if (err.detail.includes("manager_id")) {
            setError("manager_id", {
              type: "manual",
              message: "This Manager ID is already taken. Please use a different ID."
            });
        ...
        throw new Error(err.detail || "Failed to add Manager");
      }
      reset();
      toast.success('Manager added successfully!', { position: 'bottom-right' });
      if (onSuccess) onSuccess();
    } catch (error: unknown) {
      if (error instanceof Error) {
        alert(error.message || "Error adding Manager");
      } else {
      ...
      <div className="w-full max-w-lg mx-auto bg-white dark:bg-gray-900 rounded-2xl p-6 flex flex-col items-center transition-colors">
        <div className="flex flex-col items-center mb-6 w-full">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Add Manager</h2>
          </div>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
          {/* Manager ID */}
          <div>
            <label className="block font-semibold text-gray-700 dark:text-gray-200 mb-1 flex items-center gap-1" htmlFor="manager_id">
              <FaIdBadge className="text-blue-400" /> Manager ID
              {isCheckingManagerId && <span className="text-xs text-blue-500 ml-2">Checking...</span>}
            </label>
            <input
              id="manager_id"
              type="text"
              {...register("manager_id")}
              className={`w-full px-4 py-2 rounded-lg border ${errors.manager_id ? "border-red-500" : "border-gray-300 dark:border-gray-700"} bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 ${errors.manager_id ? "focus:ring-red-500" : "focus:ring-blue-500"} transition`}
              placeholder="Manager ID"
            />
            {errors.manager_id && <span className="text-xs text-red-500 flex items-center gap-1 mt-1"><FaTimesCircle /> {errors.manager_id.message}</span>}
          </div>
          {/* Name */}
          <div>
