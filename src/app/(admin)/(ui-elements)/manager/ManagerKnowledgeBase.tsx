...
export default function ManagerKnowledgeBase() {
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg mb-3 overflow-hidden transition-all duration-200">
        ...
        {isExpanded && (
          <div className="px-4 pb-4 space-y-2">
            ...
            <div className="pt-2">
              <a
                href={`${BASE_URL}/domain/domain_5/file/${encodeURIComponent(doc.name)}`}
                target="_blank"
  ...
  const handleRefreshClick = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/domain/domain_5/files?page=1&page_size=10`, {
        method: "GET",
        headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`${BASE_URL}/domain/domain_5/files`, {
        method: "DELETE",
        headers: {
          accept: "application/json",
  ...
  const handleConfirmUpload = async () => {
    if (selectedFiles.length > 0) {
      ...
      const uploadUrl = `${BASE_URL}/domain/domain_5/upload`;
      try {
