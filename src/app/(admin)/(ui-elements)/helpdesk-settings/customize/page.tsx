...
const CustomizePage = () => {
    ...
    // Mobile view toggle between Customize (left) and Preview (right)
    ...
    // Removed unused: error, successMessage
    ...
    // Load customer ticket form configuration on component mount
    useEffect(() => {
    ...
    const loadCustomerTicketFormConfig = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Map the response data to form configuration
                const mergedConfig = {
                    title: config.title || "Customer Ticket",
                    fields: {
                        id: {
                            label: config.fields?.id?.label || "Email",
                            placeholder: config.fields?.id?.placeholder || "abc@example.com"
                        },
                        phone: {
                        ...
                        issueType: {
                            label: config.fields?.issueType?.label || "Issue",
                            options: config.fields?.issueType?.options || ["Delay", "Payment", "Working"]
                        },
                        issue: {
                            label: config.fields?.issue?.label || "Issue",
                            options: config.fields?.issue?.options || ["Delay Delivery", "Payment Problem"]
                        },
                        message: {
                            label: config.fields?.message?.label || "Message",
                            placeholder: config.fields?.message?.placeholder || "Please Describe your Problem here"
                        }
                    },
                    submitButtonText: config.submitButtonText || "Submit Ticket",
                    successMessage: config.successMessage || "We have received your query."
                };
                setCustomerTicketFormConfig(mergedConfig);
                showAlert('Customer ticket form configuration loaded successfully.', 'success');
    ...
    // Customer Ticket Form Configuration State
    const [customerTicketFormConfig, setCustomerTicketFormConfig] = useState({
        title: "Customer Ticket",
        fields: {
            id: { label: "Email", placeholder: "abc@example.com" },
            phone: { label: "Phone Number", placeholder: "1234567890" },
            issueType: { label: "Issue", options: ["Delay", "Payment", "Working"] },
            issue: { label: "Issue", options: ["Delay Delivery", "Payment Problem"] },
            message: { label: "Message", placeholder: "Please Describe your Problem here" }
        },
        submitButtonText: "Submit Ticket",
        successMessage: "We have received your query."
    });
  ...
  useEffect(() => {
    ...
    // Removed unused: handleThemeSelect
    ...
    // Removed unused: handleOptionChange

    // Removed unused: addOption

    // Removed unused: removeOption
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <div className="p-4 sm:p-6 max-w-7xl mx-auto">
                <DashboardHeader
                    variant="default"
                    size="lg"
                    title="Customize Customer Ticket Form"
                    subtitle="Configure fields for the customer ticket form and preview the chatbot experience in real-time"
                    breadcrumbs={[
                        { label: 'Home', href: '/' },
                        { label: 'HelpDesk Settings', href: '/helpdesk-settings' },
                        { label: 'Customize Customer Ticket Form', href: '/helpdesk-settings/customize' }
                    ]}
                    icon={() => (
                    ...
                    <div className={`${activeMobileTab === 'customize' ? 'block' : 'hidden'} xl:block w-full lg:w-full bg-white dark:bg-gray-900 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700`}>
                        ...
                        <div className="space-y-6 max-h-[70vh] md:max-h-[600px] overflow-y-auto">
                            ...
                            <div className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                Phone numbers should accept 8-15 digits (numbers only)
                            </div>
                        </div>
