...
type SendNowResponse = {
  ...
  tracking?: {
    unique_id?: string;
    dashboard_data_snapshot?: DashboardDataSnapshot;
  };
};
...
const [sendingEmails, setSendingEmails] = useState<Set<string>>(new Set());
 ...
 const [rowPendingDelete, setRowPendingDelete] = useState<{ row: DashboardSettingRow; index: number } | null>(null);
 // Pagination state
 ...
 // Build a stable unique key for a row; prefer id, else include index fallback
 const getRowKey = (row: DashboardSettingRow, rowIndex?: number) =>
   (row.id !== undefined && row.id !== null)
     ? `id_${row.id}`
     : `${row.email}_${row.dashboardType}_${row.frequency}_${row.time}_${row.schedule_value || 'default'}_${rowIndex ?? '0'}`;
 // Map common IANA names to provided abbreviations when defaulting
 const ianaToAbbrevMap = useCallback((): Record<string, string> => ({
   "Asia/Kolkata": "IST",
   "Europe/London": "GMT",
 ...
 useEffect(() => {
   (async () => {
     try {
       ...
       if (currentTz) {
         const map = ianaToAbbrevMap();
         const fromIana = map[currentTz.timezone as string] || map[currentTz.timezone_name as string];
 ...
 const openDeleteModal = (row: DashboardSettingRow, index: number) => {
   setRowPendingDelete({ row, index });
 ...
 const fetchSettings = () => {
   fetch((process.env.NEXT_PUBLIC_API_URL) + "/dashboard/dashboard-settings")
    ...
    .then(data => {
      ...
      setSettings(rows.slice().reverse());
    });
 };
 useEffect(() => {
   fetchSettings();
 ...
 try {
   ...
   // Call schedule-email (backend also persists to MongoDB)
   // console.log('Making request to:', `${process.env.NEXT_PUBLIC_API_URL}/dashboard/schedule-email");
   ...
   if (!scheduleRes.ok) {
     ...
     if (shouldRetryWithoutTimezone) {
                try {
          const retryParams = { ...scheduleParams } as Record<string, unknown>;
          ...
          if (retryRes.ok) {
            ...
            showToast("success", "Success", "Email scheduled and saved successfully.");
            fetchSettings();
            setIsSubmitting(false);
            return;
          }
   ...
   if (scheduleData.error) {
     showToast("error", "Schedule Error", scheduleData.error);
       setIsSubmitting(false);
       return;
     }
   showToast("success", "Success", "Email scheduled and saved successfully.");
 ...
 const isFormInvalid = !!emailError || !!timeError || !!freqError || !email.trim() || (frequency === 'monthly' && !scheduleValue) || (frequency === 'weekly' && !weekDay) || (frequency === 'interval' && (parseInt(intervalHours) === 0 && parseInt(intervalMinutes) === 0));
 ...
 const handleDelete = async (email: string, row?: DashboardSettingRow, rowIndex?: number) => {
   hideToast();
   try {
     let res: Response | null = null;
     // 1) Prefer delete by id if available
     if (row && row.id !== undefined && row.id !== null) {
       const id = String(row.id);
       const candidates = [
         (process.env.NEXT_PUBLIC_API_URL) + "/dashboard/dashboard-settings/id/" + encodeURIComponent(id),
         (process.env.NEXT_PUBLIC_API_URL) + "/dashboard/dashboard-settings/" + encodeURIComponent(id) + "?type=id"
       ];
       for (const url of candidates) {
         try {
           const attempt = await fetch(url, { method: 'DELETE', headers: { 'accept': 'application/json' } });
           if (attempt.ok || attempt.status === 404 || attempt.status === 200 || attempt.status === 204) {
             res = attempt;
             break;
           }
         } catch {}
     ...
     // 2) Fallback to legacy delete by email
     if (!res) {
       res = await fetch((process.env.NEXT_PUBLIC_API_URL) + "/dashboard/dashboard-settings/" + encodeURIComponent(email), {
         method: 'DELETE',
         headers: { 'accept': 'application/json' },
       });
     }
     ...
     // Treat 404 as already deleted: proceed to remove locally
     if (!res.ok && res.status !== 404) {
       showToast("error", "Error", (data && data.error) || responseText || `Failed to delete setting. (Status: ${res.status})`);
       return;
     }
     showToast("success", "Success", "Deleted successfully.");
     // Remove only the clicked row locally (by absolute index first, then id/composite)
     setSettings(prev => {
       const next = [...prev];
       if (row) {
         let removeIdx = -1;
         if (typeof rowIndex === 'number') {
           const absoluteIdx = startIndex + rowIndex;
           if (absoluteIdx >= 0 && absoluteIdx < next.length) {
             removeIdx = absoluteIdx;
           }
         }
         if (removeIdx === -1 && row.id !== undefined && row.id !== null) {
           removeIdx = next.findIndex(r => r.id === row.id);
         }
         if (removeIdx === -1) {
           removeIdx = next.findIndex(r => r.email === row.email && r.dashboardType === row.dashboardType && r.frequency === row.frequency && r.time === row.time && (r.schedule_value || 'default') === (row.schedule_value || 'default'));
         }
...
const handleSendNow = async (row: DashboardSettingRow, rowIndex?: number) => {
  hideToast();
  
  // Add unique row identifier to sending set to show loading state
  const uniqueRowId = getRowKey(row, rowIndex);
  ...
  try {
    ...
    if (row.frequency === 'weekly' && row.schedule_value) {
      sendNowParams.day_of_week = row.schedule_value;
    }
    if (row.frequency === 'monthly' && row.schedule_value) {
      sendNowParams.day_of_month = parseInt(row.schedule_value || '1');
    }
    if (row.frequency === 'interval' && row.schedule_value) {
      const [hours, minutes] = row.schedule_value.split(':');
      sendNowParams.interval_hours = parseInt(hours || '0');
      sendNowParams.interval_minutes = parseInt(minutes || '0');
    }
    ...
    const res = await fetch((process.env.NEXT_PUBLIC_API_URL) + "/dashboard/send-now", {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        "Accept": "application/json"
      },
      body: JSON.stringify(sendNowParams),
    });
    
    console.log('Send Now Response Status:', res.status, res.statusText);

    let errorText = '';
    let data: SendNowResponse = {};
    try {
      data = await res.json();
    ...
    // Enhanced success message with backend response details
    const successMessage = data.message || "Email has been sent immediately and successfully.";
    
    // Check if email was actually sent based on backend response
    if (data.status === 'sent' && data.to) {
      showToast("success", "Email Sent", successMessage);
    ...
    showToast("error", "Error", err instanceof Error ? err.message : 'An error occurred while sending the email. Please try again later.');
  } finally {
 ...
 return (
   <>
     {/* Toast Notifications */}
     {toast.show && (
       <div className="fixed top-2 xs:top-4 right-2 xs:right-4 z-50 max-w-xs xs:max-w-sm">
     ...
     <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md w-full p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8 max-w-full mx-auto min-h-screen">
       ...
       <Modal isOpen={isDeleteModalOpen} onClose={closeDeleteModal} className="max-w-md sm:max-w-lg">
         <div className="p-6 sm:p-8">
           <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Delete Setting</h3>
           <p className="text-gray-600 dark:text-gray-300 mb-1">Are you sure you want to delete this setting?</p>
           <p className="text-red-600 dark:text-red-400 text-sm mb-6">This action cannot be undone.</p>
           <div className="flex items-center justify-end gap-3">
             ...
             <button
               onClick={() => rowPendingDelete && handleDelete(rowPendingDelete.row.email, rowPendingDelete.row, rowPendingDelete.index)}
       ...
       <style jsx>{`
         ...
         @media (max-width: 320px) {
           .ultra-small-text { font-size: 0.7rem !important; }
           .ultra-small-padding { padding: 0.25rem !important; }
           .ultra-small-gap { gap: 0.25rem !important; }
         }
      ...
      <PageHeader
        ...
        description="Configure when and how admins receive automated reports and notifications by email."
        tips={[
          "Enter a valid admin email to receive reports",
          "Choose Daily/Weekly/Monthly for fixed-time schedules",
          "Use Interval for repeated runs every X hours/minutes",
          "Set Timezone to ensure emails arrive at your local time",
          "Manage or delete saved schedules from the table below",
          "Tooltip for Frequency:Choose how often this report should be sent automatically.",
          "Delete → Remove this schedule permanently",
          "Send Now → Send this report immediately"
        ]}
        breadcrumbs={[
          { label: "Home", href: "/" },
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-3 xs:space-y-4 sm:space-y-6 mb-4 xs:mb-6 sm:mb-8">
         ...
         <div className="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6 2xl:grid-cols-6 gap-2 xs:gap-3 sm:gap-4 items-end">
           
           {/* Email Field */}
           <div className="relative col-span-1 sm:col-span-1 md:col-span-2 lg:col-span-2 xl:col-span-1 2xl:col-span-1">
             ...
             <input
               ...
               className={`border ${emailError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} focus:border-blue-500 focus:ring-2 focus:ring-blue-100 p-2 xs:p-2.5 sm:p-3 rounded-lg w-full transition outline-none text-xs xs:text-sm h-9 xs:h-10 sm:h-11 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400`}
             />
             {emailError && (
               <span className="absolute left-0 top-full mt-1 text-red-500 text-xs xs:text-xs bg-white dark:bg-gray-800 px-1 pointer-events-none z-10 max-w-full break-words">
                 {emailError}
               </span>
             )}
           </div>
           ...
           <div className="relative col-span-1">
             ...
             <button
               ...
               className="border border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 p-2 xs:p-2.5 sm:p-3 rounded-lg w-full transition outline-none bg-white dark:bg-gray-800 flex items-center justify-between text-xs xs:text-sm h-9 xs:h-10 sm:h-11 text-gray-900 dark:text-white"
             >
              <span className="truncate text-left text-gray-500 dark:text-gray-400">
                {dashboardTypes.find(dt => dt.value === dashboardType)?.label || "Enter your Dashboard"}
               </span>
               <svg className="w-3 h-3 xs:w-4 xs:h-4 ml-1 xs:ml-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                ...
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-lg rounded-lg"
              >
                {dashboardTypes.map(dt => (
                  <DropdownItem key={dt.value} onClick={() => { setDashboardType(dt.value); setIsDashboardTypeOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                    {dt.label}
                  </DropdownItem>
                ))}
              </Dropdown>
           ...
           <div className="relative col-span-1">
             ...
             >
              <span className="truncate text-gray-500 dark:text-gray-400">
                {frequency ? (frequency.charAt(0).toUpperCase() + frequency.slice(1)) : 'Enter your Schedule'}
               </span>
               <svg className="w-3 h-3 xs:w-4 xs:h-4 ml-1 xs:ml-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
               </svg>
                ...
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-lg rounded-lg"
              >
                <DropdownItem onClick={() => { setFrequency("daily"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Daily</DropdownItem>
                <DropdownItem onClick={() => { setFrequency("weekly"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Weekly</DropdownItem>
                <DropdownItem onClick={() => { setFrequency("monthly"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Monthly</DropdownItem>
                <DropdownItem onClick={() => { setFrequency("hourly"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Hourly</DropdownItem>
                <DropdownItem onClick={() => { setFrequency("minutely"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Minutely</DropdownItem>
                <DropdownItem onClick={() => { setFrequency("interval"); setIsFrequencyOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">Interval</DropdownItem>
              </Dropdown>
           ...
           <div className="relative col-span-1">
             ...
             <div className="flex gap-0.5 xs:gap-1 h-9 xs:h-10 sm:h-11">
               {/* Hour */}
               <div className="relative flex-1">
                 <button
                   type="button"
                   onClick={() => setIsHourOpen(!isHourOpen)}
                   className={`border ${timeError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} focus:border-blue-500 focus:ring-2 focus:ring-blue-100 p-1 xs:p-2 rounded-lg w-full transition outline-none bg-white dark:bg-gray-800 flex items-center justify-between text-xs xs:text-sm h-full text-gray-900 dark:text-white`}
                 >
                   {hour.padStart(2, '0')}
                   <svg className="w-3 h-3 xs:w-4 xs:h-4 ml-1 text-gray-400 dark:text-gray-500 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                     <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                   </svg>
                 </button>
                                    <Dropdown
                    ...
                    className="max-h-32 xs:max-h-40 overflow-y-auto custom-scrollbar bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-lg rounded-lg"
                  >
                    {Array.from({ length: 12 }, (_, i) => (i + 1).toString().padStart(2, '0')).map(h => (
                      <DropdownItem key={h} onClick={() => { setHour(h); setIsHourOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">{h}</DropdownItem>
                    ))}
                  </Dropdown>
               </div>
               
               <span className="self-center text-gray-500 dark:text-gray-400 font-medium text-xs xs:text-sm">:</span>
                 ...
                 >
                  ...
                  >
                    {getMinuteOptions().map(m => (
                      <DropdownItem key={m} onClick={() => { setMinute(m); setIsMinuteOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">{m}</DropdownItem>
                    ))}
                  </Dropdown>
               </div>
               ...
               <div className="relative flex-1">
                 ...
                 >
                  ...
                  >
                    {["AM", "PM"].map(ap => (
                      <DropdownItem key={ap} onClick={() => { setAmpm(ap); setIsAmpmOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">{ap}</DropdownItem>
                    ))}
                  </Dropdown>
               </div>
             </div>
           ...
           <div className="relative col-span-1">
             ...
             >
              <span className="truncate text-left text-gray-500 dark:text-gray-400">
                {timezones.find(tz => tz.value === timezone)?.label || 'Enter your Timezone'}
               </span>
               <svg className="w-3 h-3 xs:w-4 xs:h-4 ml-1 xs:ml-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
               </svg>
                ...
                className="max-h-56 overflow-y-auto custom-scrollbar bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-lg rounded-lg"
              >
                {timezones.map(tz => (
                  <DropdownItem key={tz.value} onClick={() => handleTimezoneSelect(tz.value)} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                    {tz.label}
                  </DropdownItem>
                ))}
              </Dropdown>
           ...
           {/* Save Button */}
           <div className="flex items-end col-span-1 sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1">
             <Button
               onClick={() => handleSubmit(new Event('submit'))}
               disabled={isFormInvalid || isSubmitting}
               variant="primary"
               size="md"
               className="w-full responsive-button px-3 xs:px-4 sm:px-6 lg:px-8 py-2 xs:py-2.5 sm:py-3 text-xs xs:text-sm font-medium h-9 xs:h-10 sm:h-11"
             >
               ...
               <div className="w-full xs:w-48">
                 ...
                 <div className="relative">
                   <button
                     ...
                     className={`border ${freqError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} focus:border-blue-500 focus:ring-2 focus:ring-blue-100 p-2 xs:p-2.5 sm:p-3 rounded-lg w-full transition outline-none bg-white dark:bg-gray-800 flex items-center justify-between text-xs xs:text-sm h-9 xs:h-10 sm:h-11 text-gray-900 dark:text-white`}
                   >
                     {weekDay}
                     <svg className="w-3 h-3 xs:w-4 xs:h-4 ml-2 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                       <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                     </svg>
                   </button>
                                        <Dropdown
                    ...
                    >
                      {weekDays.map(day => (
                        <DropdownItem key={day} onClick={() => { setWeekDay(day); setIsWeekDayOpen(false); }} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                          {day}
                        </DropdownItem>
                      ))}
                    </Dropdown>
                 ...
                 <div className="flex gap-2 xs:gap-4">
                   <div className="w-full xs:w-24">
                     ...
                     <input
                       ...
                       className="border border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 p-2 xs:p-2.5 sm:p-3 rounded-lg w-full transition outline-none text-xs xs:text-sm h-9 xs:h-10 sm:h-11 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                       placeholder="0"
                     />
                   </div>
                   <div className="w-full xs:w-24">
       ...
       <h2 className="text-2xl font-sans text-gray-900 dark:text-white mt-4 xs:mt-6 sm:mt-8 mb-3 xs:mb-4 sm:mb-6">
        Saved Email Report & Notification Schedules
       </h2>
       
       {/* Desktop Table */}
       <div className="desktop-table overflow-x-auto rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] mt-8">
         <table className="min-w-full text-sm text-left text-gray-900 dark:text-white">
           <thead className="border-b border-gray-100 dark:border-white/[0.05] bg-gray-50 dark:bg-gray-800">
             <tr>
               <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Email</th>
              <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Dashboard Type</th>
              <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Schedule</th>
               <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Time</th>
               <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Timezone</th>
              <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-center">Action</th>
             </tr>
           </thead>
           <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                 ...
                 <tr key={row.id || index} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                   <td className="px-4 py-3 font-medium text-gray-900 dark:text-white break-all">{row.email}</td>
                  ...
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300">
                   ...
                   <td className="px-4 py-3 text-gray-700 dark:text-gray-300 font-mono">{row.time}</td>
                   <td className="px-4 py-3 text-gray-700 dark:text-gray-300">
                     {row.timezone ? (
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2 w-full">
                      ...
                      <button
                        onClick={() => handleSendNow(row, index)}
                        disabled={sendingEmails.has(getRowKey(row, index))}
                        className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-200 hover:bg-green-50 transition group disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Send Now"
                      >
                        {sendingEmails.has(getRowKey(row, index)) ? (
                          <svg className="animate-spin h-5 w-5 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                          </svg>
                        ) : (
                          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-green-500 group-hover:text-green-600">
                     ...
                     <button
                       ...
                       className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-200 hover:bg-red-50 transition group"
                       title="Delete"
                     >
                       <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-red-500 group-hover:text-red-600">
       ...
       {/* Mobile Table */}
       <div className="mobile-table space-y-3 xs:space-y-4">
         {currentData.length > 0 ? (
           currentData.map((row, index) => (
             <div key={row.id || index} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 xs:p-4 space-y-3 shadow-sm">
               {/* Header with Avatar and Email */}
               <div className="flex items-center gap-3">
                 <div className="w-8 h-8 xs:w-10 xs:h-10 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center flex-shrink-0">
                   <span className="text-gray-600 dark:text-gray-300 font-semibold text-sm xs:text-base">
                     {row.email.charAt(0).toUpperCase()}
                   </span>
                 </div>
                 <div className="flex-1 min-w-0">
                   <p className="font-medium text-gray-800 dark:text-white text-sm xs:text-base break-all">
                     {row.email}
                   </p>
                 </div>
               </div>
               ...
               <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 text-sm">
                 <div>
                   <span className="text-gray-500 dark:text-gray-400 text-xs xs:text-sm">Dashboard Type:</span>
                   <div className="mt-1">
                     <Badge size="sm" color={getDashboardBadgeColor(row.dashboardType || "")}>
                 ...
                 <div>
                   <span className="text-gray-500 dark:text-gray-400 text-xs xs:text-sm">Frequency:</span>
                   <div className="mt-1">
                     <Badge
                       size="sm"
                       color={
                 ...
                 <div>
                   <span className="text-gray-500 dark:text-gray-400 text-xs xs:text-sm">Time:</span>
                   <p className="mt-1 text-gray-800 dark:text-white font-mono text-sm">{row.time}</p>
                 </div>
                 
                 <div>
                   <span className="text-gray-500 dark:text-gray-400 text-xs xs:text-sm">Timezone:</span>
                   <div className="mt-1">
                     {row.timezone ? (
                       <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                         {timezones.find(tz => tz.value === row.timezone)?.label || row.timezone}
               ...
               <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <div className="flex gap-2 justify-center">
                  <Button
                    ...
                    variant="primary"
                    size="sm"
                    className="flex-1 !bg-green-500 !hover:bg-green-600 !text-white !border-green-500 !hover:border-green-600 text-sm py-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {sendingEmails.has(getRowKey(row, index)) ? (
                 ...
                 <Button
                   ...
                   variant="primary"
                   size="sm"
                    className="flex-1 !bg-red-500 !hover:bg-red-600 !text-white !border-red-500 !hover:border-red-600 text-sm py-2"
                 >
                    Delete
                 </Button>
                </div>
           ...
           <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 xs:p-8 text-center">
             <div className="flex flex-col items-center gap-3">
               <svg className="w-16 h-16 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
               </svg>
               <div>
                 <p className="text-gray-400 dark:text-gray-300 text-base xs:text-lg font-medium">No dashboard settings found</p>
                 <p className="text-gray-300 dark:text-gray-500 text-sm xs:text-base mt-1">Create your first email schedule using the form above</p>
               </div>
             </div>
           </div>
         )}
