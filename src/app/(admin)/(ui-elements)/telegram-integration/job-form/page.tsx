...
const JOB_FORM_API = `${API_PREFIX}/admin/job-form`;
const UI_TEXTS_API = `${API_PREFIX}/admin/job-form/ui-texts`;
const JOB_FAREWELL_API = `${API_PREFIX}/admin/job-form/job-farewell`;
const BASIC_TEXTS_API = `${API_PREFIX}/admin/job-form/texts`;
const JOB_TRIGGERS_API = `${API_PREFIX}/admin/job-triggers`;
...
export default function JobFormManager(): React.JSX.Element {
  ...
  // Job Triggers CRUD - Fetch triggers
  const fetchJobTriggers = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch job triggers');
      }
      ...
      setAlertInfo({ 
        ...
        variant: 'error', 
        title: 'Error', 
        message: 'Failed to load job triggers.' 
      });
    } finally {
  ...
  const fetchJobFormData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setJobFormData({ job_fields: [], job_prompts: [] });
      } else {
  ...
  const fetchUITextsData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUITextsData({ job_role: { body: "", footer: "" }, experience: { body: "", footer: "" } });
      } else {
  ...
  const fetchFarewellData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('No job farewell data found yet. Starting with empty list.');
        setFarewellData({ contact: { email: '', phone: '' }, messages: [] });
      } else {
  ...
  const fetchBasicTexts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('No basic texts found yet. Using defaults.');
        setBasicTexts({ job_start_text: "" });
      } else {
