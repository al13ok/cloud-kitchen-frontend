...
const EmployeeUploadModal: React.FC<EmployeeUploadModalProps> = ({ isOpen, onClose, onUploadSuccess, onError }) => {
  ...
  const handleDownloadTemplate = async () => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/template/employees/?format=excel`;
    try {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/upload-employees/`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        ...
        if (onError) onError("Failed to upload employee file. Please try again.");
        onClose();
      }
    } catch {
      if (onError) onError("Failed to upload employee file. Please try again.");
      ...
      <div className="w-full max-w-2xl mx-auto bg-white dark:bg-[#181E2A] rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-lg max-h-[90vh] md:max-h-[80vh]">
        ...
        <div className="flex-1 flex flex-col items-center justify-between p-4 sm:p-8 bg-gradient-to-b from-blue-50/60 to-white dark:from-blue-900/30 dark:to-[#181E2A] min-h-[180px]">
          ...
          <Button
            ...
            className="w-full rounded-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 shadow-sm transition flex items-center justify-center"
          >
